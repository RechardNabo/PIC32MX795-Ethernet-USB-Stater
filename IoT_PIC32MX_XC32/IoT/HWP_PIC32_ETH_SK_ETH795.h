...
// Set configuration fuses (but only in MainDemo.c where THIS_IS_STACK_APPLICATION is defined)
	...
	#pragma config FPLLODIV = DIV_1, FPLLMUL = MUL_20, FPLLIDIV = DIV_2, FWDTEN = OFF, FPBDIV = DIV_1, POSCMOD = EC, FNOSC = PRIPLL, CP = OFF
	#pragma config FMIIEN = OFF, FETHIO = ON, ICESEL = ICS_PGx1,FSOSCEN = ON,FCANIO=OFF	// external PHY in RMII/alternate configuration
#endif
// FETHIO =OFF, POSCMOD=XT for dev kit.
...
#define EnterBootTris           (TRISEbits.TRISE2)
#define UserButton               (PORTEbits.RE2)

#define Input1Tris           (TRISEbits.TRISE5)
#define Input1               (PORTEbits.RE5)
#define Input2Tris           (TRISEbits.TRISE6)
#define Input2               (PORTEbits.RE6)
#define On                      1
#define Off                     0

#define SPIFLASH_CS_TRIS                (TRISDbits.TRISD2)
#define SPIFLASH_CS_IO                  (LATDbits.LATD2)
...
// LEDs
#define LEDStatus                       (LATEbits.LATE9)
#define TrisLEDStatus                   (TRISEbits.TRISE9)
...
#define UARTTX_TRIS			(TRISDbits.TRISD15)
#define UARTRX_TRIS			(TRISDbits.TRISD14)
