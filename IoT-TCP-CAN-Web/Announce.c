...
void DiscoveryTask(void)
{
	...
	switch(DiscoverySM)
	{
		...
		case DISCOVERY_REQUEST_RECEIVED:
            ...
            strcpy((char*)ControllerName,(ROM void*)Board.ControllerName);
            strcat((char*)ControllerName,", Code version ");
            strcat((char*)ControllerName,CodeVersion);
            UDPPutROMString((ROM BYTE*)ControllerName);
			// Send the packet
			UDPFlush();
