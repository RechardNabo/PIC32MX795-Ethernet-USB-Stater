...
void SNMPUDPClient(void);
void PingClientTask(void);
void UPDChangeIPTask(void);
void SNMPServer(TypeDefSNMP *object);
...
void StackApplications(void)
{
    ...
    Board.ModbusUsedSockets=0;
    
    for(i=0;i<NumberOfModbusServersSockets;i++)
    {
        //GenericTCPServer(&Servers[i]);
        // Count the amount of connections we have
        if(Servers[i].IsConnected)
            Board.ModbusUsedSockets++;
    }
    ...
    #if defined(STACK_USE_UPDChangeIP)
    UPDChangeIPTask();
    #endif
...
//    for(i=0;i<NumberOfSNMPServersSockets;i++)
//        SNMPServer(&SNMPServers[i]);
}
